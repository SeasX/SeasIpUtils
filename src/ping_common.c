...
ping_exception *common_throw_exception(int type, const char *format, ...)
{
    va_list args;
    char *message = NULL;
    ping_exception *exception = (ping_exception *)emalloc(sizeof(ping_exception));


    va_start(args, format);
    vspprintf(&message, 0, format, args);

    exception->exception_code = type;
    spprintf(&exception->exception_message, 0, "%s", message);
...
char *char_init()
{
    char *source = (char *)emalloc(sizeof(char)*256);
    memset(source, 0x00, sizeof(char));

    return source;
}

int char_append(char *source, const char *format, ...)
{
    va_list args;
    char *message = NULL;
...
int ping_result_append(ping_result *result, char *str)
{
    spprintf(&(result->raw_result[result->raw_result_length]), 0, "%s", str);
...
void limit_capabilities(void)
{
    ...
    {
        php_printf("ping: cap_set_proc");
        exit(-1);
    }

    if (prctl(PR_SET_KEEPCAPS, 1) < 0)
    {
        php_printf("ping: prctl");
        exit(-1);
    }
...
static void fill(char *patp)
{
    ...
    for (cp = patp; *cp; cp++)
    {
        if (!isxdigit(*cp))
        {
            php_printf("ping: patterns must be specified as hex digits.\n");
            exit(2);
        }
    }
    ii = sscanf(patp,
...
void common_options(int ch)
{
    ...
    case 'c':
        ...
        if (npackets <= 0)
        {
            php_printf("ping: bad number of packets to transmit.\n");
            exit(2);
        }
        break;
    case 'd':
    ...
    case 'i':		/* wait between sending packets */
    {
        ...
        {
            php_printf("ping: bad timing interval\n");
            exit(2);
        }
    ...
    case 'm':
    {
        ...
        if (mark < 0 || *endp != '\0')
        {
            php_printf("mark cannot be negative\n");
            exit(2);
        }
        options |= F_MARK;
        break;
    ...
    case 'l':
        ...
        if (preload <= 0)
        {
            php_printf("ping: bad preload value, should be 1..%d\n", MAX_DUP_CHK);
            exit(2);
        }
        ...
        if (uid && preload > 3)
        {
            php_printf("ping: cannot set preload to value > 3\n");
            exit(2);
        }
        break;
    case 'O':
    ...
    case 'S':
        ...
        if (sndbuf <= 0)
        {
            php_printf("ping: bad sndbuf value.\n");
            exit(2);
        }
        break;
    case 'f':
    ...
    case 's':		/* size of packet to send */
        ...
        if (datalen < 0)
        {
            php_printf("ping: illegal negative packet size %d.\n", datalen);
            exit(2);
        }
        if (datalen > maxpacket - 8)
        {
            php_printf("ping: packet size too large: %d\n", datalen);
            exit(2);
        }
        break;
    case 'v':
    ...
    case 't':
        ...
        if (ttl < 0 || ttl > 255)
        {
            php_printf("ping: ttl %u out of range\n", ttl);
            exit(2);
        }
        break;
    case 'U':
    ...
    case 'W':
        ...
        {
            php_printf("ping: bad linger time.\n");
            exit(2);
        }
        lingertime *= 1000;
        break;
    case 'V':
        php_printf("ping utility, iputils-%s\n", SNAPSHOT);
        exit(0);
    default:
        abort();
    }
 ...
 * Print timestamp
 */
void print_timestamp(char *result_tmp)
{
    if (options & F_PTIMEOFDAY)
    {
        ...
        char_append(result_tmp, "[%lu.%06lu] ", (unsigned long)tv.tv_sec, (unsigned long)tv.tv_usec);
...
ping_exception *setup(int icmp_sock)
{
    int hold;
    struct timeval tv;
    sigset_t sset;
    ...
    if (uid && interval < MINUSERINTERVAL)
    {
        return ping_throw_exception(SEASIPUTILS_EXCEPTION_PROCESS_ERROR, "ping: cannot flood; minimal interval, allowed for user, is %dms\n", MINUSERINTERVAL);
    ...
    {
        return ping_throw_exception(SEASIPUTILS_EXCEPTION_PROCESS_ERROR, "ping: illegal preload and/or interval\n");
...
ping_exception *main_loop(int icmp_sock, __u8 *packet, int packlen, ping_result *result)
{
    char addrbuf[128];
    char ans_data[4096];
    struct iovec iov;
    ...
    for (;;)
    {
        ...
        for (;;)
        {
            ...
            else
            {
                ...
                not_ours = parse_reply(&msg, cc, addrbuf, recv_timep, result);
            }
...
int gather_statistics(__u8 *icmph, int icmplen,
                      ...
                      void (*pr_reply)(__u8 *icmph, int cc, char *result_tmp), ping_result *result)
{
    int dupflag = 0;
    long triptime = 0;
...
restamp:
    ...
    else
    {
        ...
        print_timestamp(tmp);
        char_append(tmp, "%d bytes from %s:", cc, from);
        // printf("%d bytes from %s:", cc, from);

        if (pr_reply)
        {
            pr_reply(icmph, cc, tmp);
        }

        if (hops >= 0)
        {
            char_append(tmp, " ttl=%d", hops);
            // printf(" ttl=%d", hops);
        }

        if (cc < datalen+8)
        {
            char_append(tmp, " (truncated)\n");
            // printf(" (truncated)\n");
        ...
        if (timing)
        {
            if (triptime >= 100000)
            {
                char_append(tmp, " time=%ld ms", triptime/1000);
                // printf(" time=%ld ms", triptime/1000);
            }
            else if (triptime >= 10000)
            {
                char_append(tmp, " time=%ld.%01ld ms", triptime/1000, (triptime%1000)/100);
                // printf(" time=%ld.%01ld ms", triptime/1000, (triptime%1000)/100);
            }
            else if (triptime >= 1000)
            {
                char_append(tmp, " time=%ld.%02ld ms", triptime/1000, (triptime%1000)/10);
                // printf(" time=%ld.%02ld ms", triptime/1000, (triptime%1000)/10);
            }
            else
            {
                char_append(tmp, " time=%ld.%03ld ms", triptime/1000, triptime%1000);
 ...
 */
ping_exception *finish(ping_result *result)
{
    ...
    char_append(tmp, "--- %s ping statistics ---\n", hostname);
    // printf("--- %s ping statistics ---\n", hostname);

    char_append(tmp, "%ld packets transmitted, ", ntransmitted);
    // printf("%ld packets transmitted, ", ntransmitted);
    result->ntransmitted = ntransmitted;

    char_append(tmp, "%ld received", nreceived);
    // printf("%ld received", nreceived);
    result->nreceived = nreceived;
    if (nrepeats)
    {
        char_append(tmp, ", +%ld duplicates", nrepeats);
    ...
    if (nchecksum)
    {
        char_append(tmp, ", +%ld corrupted", nchecksum);
        // printf(", +%ld corrupted", nchecksum);
        result->nchecksum = nchecksum;
    }
    else
    {
        result->nchecksum = 0;
    ...
    if (nerrors)
    {
        char_append(tmp, ", +%ld errors", nerrors);
    ...
    if (ntransmitted)
    {
        ...
        result->packet_loss = (int) ((((long long)(ntransmitted - nreceived)) * 100) / ntransmitted);
        char_append(tmp, ", %d%% packet loss", result->packet_loss);

        // printf(", time %ldms", 1000*tv.tv_sec+tv.tv_usec/1000);
        result->time_ms = 1000*tv.tv_sec+tv.tv_usec/1000;
        char_append(tmp, ", time %ldms\n", result->time_ms);
    }
    // putchar('\n');
    ...
    if (nreceived && timing)
    {
        ...
        spprintf(&result->rtt_min_ms, 0, "%ld.%03ld", (long)tmin/1000, (long)tmin%1000);
        spprintf(&result->rtt_avg_ms, 0, "%ld.%03ld", (unsigned long)(tsum/1000), (long)(tsum%1000));
        spprintf(&result->rtt_max_ms, 0, "%ld.%03ld", (long)tmax/1000, (long)tmax%1000);
        spprintf(&result->rtt_mdev_ms, 0, "%ld.%03ld", (long)tmdev/1000, (long)tmdev%1000);

        char_append(tmp, "rtt min/avg/max/mdev = %s/%s/%s/%s ms",
                    result->rtt_min_ms, result->rtt_avg_ms, result->rtt_max_ms, result->rtt_mdev_ms);
    ...
    if (pipesize > 1)
    {
        char_append(tmp, "%spipe %d", comma, pipesize);
    ...
    if (nreceived && (!interval || (options&(F_FLOOD|F_ADAPTIVE))) && ntransmitted > 1)
    {
        ...
        // printf("%sipg/ewma %d.%03d/%d.%03d ms", comma, ipg/1000, ipg%1000, rtt/8000, (rtt/8)%1000);
        spprintf(&result->ipg_ms, 0, "%d.%03d", ipg/1000, ipg%1000);
        spprintf(&result->ewma_ms, 0, "%d.%03d", rtt/8000, (rtt/8)%1000);
        char_append(tmp, "%sipg/ewma %s/%s ms", result->ipg_ms, result->ewma_ms);
    }
    else
    {
        spprintf(&result->ipg_ms, 0, "0");
        spprintf(&result->ewma_ms, 0, "0");
    }
    // putchar('\n');
