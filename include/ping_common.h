#ifndef _SEASIPUTILS_PING_COMMON_H_
#define _SEASIPUTILS_PING_COMMON_H_
...
{
    return modify_capability(CAP_NET_RAW,   CAP_SET);
};
static inline int disable_capability_raw(void)
{
    return modify_capability(CAP_NET_RAW,   CAP_CLEAR);
};
static inline int enable_capability_admin(void)
{
    return modify_capability(CAP_NET_ADMIN, CAP_SET);
};
static inline int disable_capability_admin(void)
{
    return modify_capability(CAP_NET_ADMIN, CAP_CLEAR);
};
#else
extern int modify_capability(int);
static inline int enable_capability_raw(void)
{
    return modify_capability(1);
};
static inline int disable_capability_raw(void)
{
    return modify_capability(0);
};
static inline int enable_capability_admin(void)
{
    return modify_capability(1);
...
extern int parse_reply(struct msghdr *msg, int len, void *addr, struct timeval *tv, ping_result *result);
...
extern ping_exception *main_loop(int icmp_sock, __u8 *buf, int buflen, ping_result *result);
extern ping_exception *finish(ping_result *result);
extern void status(void);
extern void common_options(int ch);
extern int gather_statistics(__u8 *ptr, int icmplen,
                             int cc, __u16 seq, int hops,
                             int csfailed, struct timeval *tv, char *from,
                             void (*pr_reply)(__u8 *ptr, int cc, char *result_tmp), ping_result *result);
extern void print_timestamp(char *result_tmp);
...
#define ping_throw_exception(_type_, ...) common_throw_exception(_type_, __VA_ARGS__)

#endif /* _SEASIPUTILS_PING_COMMON_H_ */